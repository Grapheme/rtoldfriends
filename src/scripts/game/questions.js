Game.questions = {
  ...
  finish: function() {
    ...
    share.title = win ? 'Я отлично помню своих друзей! В частности:' : 'Я совсем забыл своих друзей ...';
    var friendsIds = _.chain(win ? t.unforgottenFriends : t.forgottenFriends).uniq().map(function(id) { return '@id' + id; }).value();
    share.title += win ? ' ' + friendsIds.join(', ') + '. Теперь ваша очередь пройти тест!' : ' ' + friendsIds.join(', ') + '.';
